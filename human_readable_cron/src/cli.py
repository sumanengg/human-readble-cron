...
def main():
    ...
    convert_parser = subparsers.add_parser(
        "convert", help="Convert human-readable cron to standard cron format"
    )
    convert_parser.add_argument(
        "expression", type=str, help="The human-readable cron expression to convert"
    )
    convert_parser.set_defaults(func=convert_command)

    # Add 'validate' subcommand (example for extensibility)
    validate_parser = subparsers.add_parser(
        "validate", help="Validate a standard cron expression"
    )
    validate_parser.add_argument(
        "expression", type=str, help="The standard cron expression to validate"
    )
